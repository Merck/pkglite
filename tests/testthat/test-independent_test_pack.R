...
testthat::test_that("pack returns the text file correctly", {
  ...
  testthat::expect_snapshot_file(save_txt(x %>% readLines()), "pack.txt")
})
...
testthat::test_that("fc_to_text returns the strings correctly", {
  local_edition(3)
  lst_fc <- list(fc1, fc2)
  testthat::expect_snapshot_file(save_txt(fc_to_text(lst_fc[[1]]) %>% unlist()), "fc_to_text1.txt")
  testthat::expect_snapshot_file(save_txt(fc_to_text(lst_fc[[2]]) %>% unlist()), "fc_to_text2.txt")
})
...
testthat::test_that("file_to_vec returns the strings correctly", {
  ...
  testthat::expect_snapshot_file(save_txt(res), "file_to_vec.txt")
})
...
testthat::test_that("read_file_text returns the strings correctly", {
  ...
  fc <- list(fc1, fc2)
  df <- fc[[1]][["df"]]
  testthat::expect_snapshot_file(save_txt(read_file_text(df[1, "path_abs"])), "read_file_text1.txt")
  testthat::expect_snapshot_file(save_txt(read_file_text(df[2, "path_abs"])), "read_file_text2.txt")
  testthat::expect_snapshot_file(save_txt(read_file_text(df[3, "path_abs"])), "read_file_text3.txt")
  testthat::expect_snapshot_file(save_txt(read_file_text(df[4, "path_abs"])), "read_file_text4.txt")
  testthat::expect_snapshot_file(save_txt(read_file_text(df[5, "path_abs"])), "read_file_text5.txt")
})
